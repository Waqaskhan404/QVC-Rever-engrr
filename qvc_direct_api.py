...
def _dates_worker(center_name, vsc_id, year, month, token, proxy_line,
                  country_id, visa_type_id, enc_visa, enc_pass, sponsor_type_ids):
    t_sess = _make_session(proxy_line)
    return get_appointment_dates(
        t_sess, token, year, month, vsc_id, country_id,
        visa_type_id, enc_visa, enc_pass, sponsor_type_ids
    )


def _slots_worker(date_str, vsc_id, country_id, visa_number, passport_number,
                  token, proxy_line, enc_visa, enc_pass, sponsor_type_ids):
    t_sess = _make_session(proxy_line)
    return fetch_slots(
        t_sess, token, date_str, vsc_id, country_id,
        visa_number, passport_number, enc_visa, enc_pass, sponsor_type_ids
    )
            ...
            while True:
                ...
                if _slot_scan_tasks:
                    with ThreadPoolExecutor(max_workers=min(len(_slot_scan_tasks), 8)) as _ex:
                        _fmap = {
                            _ex.submit(
                                _slots_worker,
                                date_str, cvsc["vscId"], country_id,
                ...
                for center_name, center_vsc in VSC_MAP.items():
                    ...
                    for (yr, mo) in year_months:
                        ...
                        if not dates:
                            print(f"[SCAN] No dates in {calendar.month_name[mo]} {yr} at {center_name}")
                            continue
                        print(f"[SCAN] {calendar.month_name[mo]} {yr} → {center_name}: {', '.join(dates)}")
                        for date_str in dates:
